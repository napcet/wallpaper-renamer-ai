...
# def limpar_nome(nome):
    ...
    palavras_final = palavras_unicas[:2]

    nome_final = "-".join(palavras_final)
    return nome_final[:20]  # ainda limitamos para evitar nomes longos demais

def limpar_nome(nome):
    """
    Processa a descrição para criar um nome fixo de 20 caracteres,
    removendo palavras comuns e duplicadas.
    """
    stopwords = {"a", "the", "of", "in", "on", "with", "and", "to", "at", "by", "for", "an", "into", "over", "under", "from"}
    nome = nome.lower()
    palavras = re.findall(r'\b\w+\b', nome)
    ...
    palavras_final = palavras_unicas[:4]  # tente até 4 palavras curtas
    nome_base = "-".join(palavras_final)

    # Ajustar o nome final para exatamente 20 caracteres
    if len(nome_base) > 25:
        nome_final = nome_base[:25]
    else:
        nome_final = nome_base.ljust(25, "-")  # completa com hífens
...
def renomear_wallpapers():
    """
    Processa todas as imagens na pasta 'wallpapers/', gera nomes descritivos,
    converte as imagens para JPEG e salva na pasta 'renomeados/' com nomes padronizados.
    """
    contador = 1
    os.makedirs(DESTINO_RENOMEADOS, exist_ok=True)
    ...
    for nome_arquivo in os.listdir(WALLPAPER_DIR):
        ...
        # Verifica se é arquivo e tem extensão válida
        if (
        ...
        try:
            ...
            novo_nome = f"{nome_limpo}-{sufixo}.jpg"  # Forçamos extensão .jpg
            novo_caminho = os.path.join(DESTINO_RENOMEADOS, novo_nome)

            # Abrir, converter e salvar como JPG
